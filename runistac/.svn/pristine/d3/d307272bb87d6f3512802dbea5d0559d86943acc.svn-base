...
var textoFirma;
 /**
  * abre el modal para firmar un documento json
  * @param {String} titulo titulo que aparece en el modal
  * @param {String} contenidoFirma texto a firmar
  * @param {Function(String)} callback funcion con parametro string que devuelve los datos firmados
  * @returns {undefined}
  */
function modalFirma(titulo, contenidoFirma, callback){
    firmaCallback=callback;
    textoFirma=contenidoFirma;
    $('#firmaTitulo').html(titulo);
    $('#textoFirma').val(textoFirma);
    $('#firmaModal').modal();
}

function aceptarFirma(){
    var datosFirmados=signDigest(textoFirma);
    if(datosFirmados){
        $('#firmaModal').modal('hide');
        firmaCallback(JSON.stringify({'firma':datosFirmados}));
    }
}
...
// Do the Sign operation
    ...
    } catch (e)
    {
        if (e.number != CAPICOM_E_CANCELLED)
        {
            alert("Ha ocurrido un error al activar la libreria de firma");
